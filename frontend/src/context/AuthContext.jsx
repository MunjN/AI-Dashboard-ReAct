...
const API_BASE = import.meta.env.VITE_API_BASE;
const APP_ID = "ai-dashboard-render"; // ✅ Render deployment identifier
const LOGIN_QUEUE_KEY = "loginQueue_v1";
...
async function flushLoginQueue(jwt, maxAttempts = 3) {
  ...
  while (q.length && attempts < maxAttempts) {
    try {
      ...
      // ✅ guarantee an origin always goes up
      const withOrigin = {
        ...evt,
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    currentUser.getSession(async (err, session) => {
      ...
      // ✅ count restore as login + include origin
      queueLoginEvent({
        appId: APP_ID,
        queuedAt: new Date().toISOString(),
        reason: "restore",
  ...
  const signIn = async ({ email, password }) => {
    ...
    // ✅ queue first so login is never lost + include origin
    queueLoginEvent({
      appId: APP_ID,
      queuedAt: new Date().toISOString(),
      reason: "signIn",
